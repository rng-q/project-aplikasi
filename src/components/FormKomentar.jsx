...
import heroImage from "../assets/FormKomentar.gif";

export default function FormKomentar() {
  const [formData, setFormData] = useState({ mahasiswa_id: "", komentar: "" });
  ...
  const handleChange = (e) => {
    ...
    try {
      // Kirim komentar (analisis langsung dari backend)
      ...
      console.log("Respons komentar:", res.data);

      const hasilSentimen = res.data.data.analisis_sentimen?.kategori || "Tidak diketahui";
      setSentimen(hasilSentimen);

      setMessage(`Komentar berhasil dikirim! Sentimen: ${hasilSentimen}`);
      setFormData({ mahasiswa_id: "", komentar: "" });
    } catch (err) {
